...
const Bond: React.FC = () => {
  ...
  const handleClickClaim = async (
    ...
    if (isPendingClaim || !pendingReward || !stakingAddress || !account) return;
    ...
    },
    {
      ...
      render: (value, data) => {
        ...
        if (typeof apr === "string") {
          ...
          const groupedApr = apr.reduce((result: any, crrApr, index) => {
            const crrRewardToken = config[index]?.rewardToken as TokenType;
            const crrAprNumber = Number(
              crrApr ? crrApr.replace(/%/gi, "").replace(/,/gi, "") : 0
            );
            return {
              ...result,
              [crrRewardToken]:
          ...
          return (
            <Flex alignItems="center" gap="10px" flexDirection="column">
              {Object.keys(groupedApr).map((rewardToken, index) => {
                ...
                item.pendingReward > 0 && (
                  <Flex>
                    ...
                    >
                      {`${addSuffix(
                        item.pendingReward
                      )} ($ ${item.priceInUsd.toLocaleString("en-US", {
          ...
          <>
            ...
            >
              {((data.balance || 0) > 0 ||
                ...
                <Text color="black">
                  {` LP Available: ${addSuffix(data.balance || 0)}`}
                </Text>
              )}
              {result.map((item, index) => {
                return (
        ...
        <Flex flexDirection="column">
          ...
          {data.token1 !== TokenType.USDC &&
            data.token1 !== TokenType.HOPERS && (
              <Text fixedFontSize=".8em" color="#777">{`1 ${getTokenName(
                data.token1
              )} ≃ ${getUSDCValue(data)}  USDC`}</Text>
            )}
      ...
      <Wrapper>
        ...
        <Table<TPool>
              ...
              case PoolType.MYPOOL:
                return (
                  ...
                  ((liquidity.bonded as number[]) &&
                    (liquidity.bonded as number[])?.find((x) => x > 0))
                );
              default:
                return false;
            }
          ...
          option={{
            ...
            tab: {
              tabs: (Object.keys(PoolType) as Array<keyof typeof PoolType>)
                .filter((x) => x !== "ALL")
